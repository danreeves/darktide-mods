...
template.show_health_bar = mod:get("show_bar")
template.show_numbers = mod:get("show_damage_numbers")
template.show_armor_types = mod:get("show_armour_type")
template.show_dps = mod:get("show_dps")
template.skip_damage_from_others = false
template.fade_bar_on_death = false
template.damage_number_type = mod:get("damage_number_type")
template.size = size
template.name = "healthbars_mod"
...
template.max_distance = mod:get("max_distance")
...
local function _flashy_damage_number_function(
	...
	for i = num_damage_numbers, 1, -1 do
		...
		if damage_number.hit_world_position then
			...
			-- MODIFIED. Assume this screen offset fixes something
			y_position = world_to_screen_position.y - mod.screen_offset_y
			x_position = world_to_screen_position.x - mod.screen_offset_x

			-- MODIFIED. This fixes position for non 1080 resolutions.
			x_position = x_position * ui_renderer.inverse_scale
			y_position = y_position * ui_renderer.inverse_scale
		end
			...
			if template.show_armor_types then
				...
				if template.damage_number_type == damage_number_types.readable then
					local armor_type_position = Vector3(
						x_position,
						y_position - damage_number_settings.has_taken_damage_timer_y_offset * 0.7,
						z_position
					)
...
template.create_widget_defintion = function(temp, scenegraph_id)
	...
	-- MODIFIED. Add support for bar and numbers both being disabled
	return UIWidget.create_definition({
		{
			pass_type = "logic",
			value = template.damage_number_function,
...
template.update_function = function(parent, ui_renderer, widget, marker, template, dt, t)
	...
	local damage_number_settings = template.damage_number_settings
	local show_damage_number = not template.skip_damage_from_others
